...
P = ParamSpec("P")
Concur_Wait_Time: Dict[str, timedelta] = {}
...
class TaskQueue:
    ...
    def put(
        ...
        **kwargs: P.kwargs
        ...
        for each in self._wait_queue:
            if _trigger_id in each:
                return {"data": {
                    "type": 0,
                    "info": {
                        "text": "您已经提交过任务，请耐心等待"
        ...
        if _trigger_id in self._concur_queue:
            return {"data": {
                "type": 0,
                "info": {
                    "text": "您已经提交过任务，请耐心等待"
        ...
        if self._wait_queue.count(_trigger_id) > 0:
            return {"data": {
                "type": 0,
                "info": {
                    "text": "您已经提交过任务，请耐心等待"
    ...
    def pop(self, _trigger_id: str) -> None:
        ...
        Concur_Wait_Time.pop(_trigger_id)
        if self._wait_queue:
            self._exec()
    ...
    def _exec(self):
        ...
        Concur_Wait_Time[key] = datetime.now()
